/* 
 * fs.sidebar_gallery
 * Updated 2017-08-16 by: Dan Lindquist
 * 
 * gallery handler
 * 
 ...
 * 
 * 2: Add this code somewhere so it runs before the gallery is output. Can be in the header or immediately before the output. Make sure it loads after jQuery and Cycle2:
	<?php
	...
	{
		?>
		<script src="<?=TEMPPATH;?>/scripts/fs.sidebar_gallery/v2017-08-16/fs.sidebar_gallery.js"></script>
		<script src="<?=TEMPPATH;?>/scripts/fs.sidebar_gallery/v2017-08-16/lib.js"></script>
		<link rel="stylesheet" href="<?=TEMPPATH;?>/scripts/fs.sidebar_gallery/v2017-08-16/lib.css" type="text/css" />
		<script type="text/javascript">
			var json_gallery = <?=json_encode($photo_gallery);?>;
			var sidegallery_perpage = 3;
		</script>
		<?php
	}//end if gallery has photos
	?>
 * 
 * 3: Add code to the page to spit out the gallery:
	<div data-featherlight-gallery data-featherlight-filter="a" id="sidebar-gallery" class="cycle-slideshow" data-cycle-swipe="true" data-cycle-slides="> div" data-cycle-pager="" data-cycle-prev="" data-cycle-next="" data-cycle-paused="true">
		<div></div>
		<div></div>
	</div>
	<div class="pager-wrapper">
		<div id="paged-pager"></div>
		<div class="sidebar-gallery-controls">
			<div class="sidebar-gallery-controls-prev">&#xf0d9;</div>
			<div class="sidebar-gallery-controls-next">&#xf0da;</div>
		</div>
	</div><!--end pager-wrapper-->
 * 
 * 4: Check to make sure that the styles for #sidebar-gallery and .pager-wrapper are in the _gallery.scss file
 */

var fs_sidebar_gallery =
{
	gallery_array: [],
	gallery: null,
	pager: null,
	slides_per_page: 4,
	slide_format: '<div><a href="{{src}}">\n{{img}}\n</a></div>\n',  // need to ramp this up for a paged pager
	...
	load: function(z) {
		if (this.gallery_array.length-1 >= z) {
			...
			for(var lp=0;lp<tgt['album_images'].length;lp++) {
				...
				newSlide = newSlide.replace(/{{src}}/i,ts['sizes']['large']);
	...
	set_slide: function(pageElementReference)
	{
		var ida = jQuery(pageElementReference).attr('id').split('-');
		var paid = ida[1];
		this.gallery.cycle('goto',paid);
	},//end function set_slide
	...
	init: function(tgt,arr,pgr,spp) {
		...
		// add handling for clicking and hovering on slides
		this.pager.on("click",".gallery-pager-slide",function(e){
			fs_sidebar_gallery.set_slide(jQuery(this));
		});
		
		//this.pager.on("mouseover",".gallery-pager-slide",function(e){
		//	fs_sidebar_gallery.set_slide(jQuery(this));
		//});
		
		
	},//end function init
};//end var fs_sidebar_gallery
...
jQuery(window).load(function()
{
	...
	{
		fs_sidebar_gallery.init("#sidebar-gallery",json_gallery,"#paged-pager",sidegallery_perpage);
		
		jQuery(window).on("resize",function(){
			fs_sidebar_gallery.center();
		});
		
		jQuery(".sidebar-gallery-controls-prev").on("click",function(){
			fs_sidebar_gallery.prev_page();
		});
		jQuery(".sidebar-gallery-controls-next").on("click",function(){
			fs_sidebar_gallery.next_page();
		});
	}//end if sidebar gallery exists
});//end jQuery load